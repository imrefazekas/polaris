...
function processMessage ( options ) {
	...
	let callerFn = options.call || call
	...
	if (_.isFunction( service )) {
		...
		try {
			let res = callerFn( argv[0], argv[1], entity, service, parameters, options )
			if (res) exit( null, res )
		} catch (err) { exit( err ) }
	}
	else
		callerFn( argv[0], argv[1], entity, service, parameters, options )
